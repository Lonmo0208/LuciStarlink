...
public final class RuntimeLightChangeBuffer {
    ...
    public void addLight(int index, int oldLight, int newLight) {
        addMaterial(index, oldLight & ImageMaterial.LIGHT_MASK, newLight & ImageMaterial.LIGHT_MASK);
    }

    public void addMaterial(int index, int oldMaterial, int newMaterial) {
        ensureCapacity(size + 1);
        oldMaterial &= ImageMaterial.MATERIAL_MASK;
        newMaterial &= ImageMaterial.MATERIAL_MASK;
        changes[size++] = pack(index, oldMaterial, newMaterial);

        int oldLight = oldMaterial & ImageMaterial.LIGHT_MASK;
        int newLight = newMaterial & ImageMaterial.LIGHT_MASK;
        int delta = oldLight ^ newLight;
        hasOpacityChange |= (delta & 0x0F) != 0;
        hasEmissionChange |= (delta & 0xF0) != 0;
        hasSkyChange |= ((oldMaterial ^ newMaterial) & ImageMaterial.SKY_RELEVANT_MASK) != 0;
    ...
    public static int oldLight(long change) {
        return oldMaterial(change) & ImageMaterial.LIGHT_MASK;
    }

    public static int newLight(long change) {
        return newMaterial(change) & ImageMaterial.LIGHT_MASK;
    }

    public static int oldMaterial(long change) {
        return (int) ((change >>> 32) & ImageMaterial.MATERIAL_MASK);
    }

    public static int newMaterial(long change) {
        return (int) ((change >>> 48) & ImageMaterial.MATERIAL_MASK);
    }
    ...
    public static boolean hasSkyChange(long change) {
        return ((oldMaterial(change) ^ newMaterial(change)) & ImageMaterial.SKY_RELEVANT_MASK) != 0;
    }

    public static boolean hasSkyMaterialChange(long change) {
        return ((oldMaterial(change) ^ newMaterial(change)) & ~ImageMaterial.LIGHT_MASK & ImageMaterial.SKY_RELEVANT_MASK) != 0;
    }
}
