...
> **[已归档 — 2026-09-21]** 这份工作单写于 1.1.4 时代，下面的"当前发布线"与性能数字都已过时；
> 发布线现在是 **1.2.10**（tag `v1.2.10`），新窗口的入口是 `docs/HANDOVER.md`，当前性能口径见
> `docs/TASK-PERF-SKY.md` §18 / §18.5。保留它是因为其中的**功能待办**（客户端光照接管、作业内并行、
> 天空档决定）仍然是待办清单。
...
- 26 测试全绿；**✅ 已通过（ef089c8 + 419f780）**
- 四项工作负载同场交错 **零回归**（≥3 轮）；**⏳ 进行中（AB_GROUP=cr1）**
- **≥3 次相邻对探针逐位一致**；**✅ 已通过 3/3**：三轮的 `edit` = `013c3846466ff9d5 / af3a8fcaba55adfd`、
  `edit_border` = `c4aed9ab13e5d3b5 / a15e9006cdb8d3a4`，与对照逐位相同 → 「面没变就不通知」没有漏掉必需的通知 ✓
  （**注意复现前提**：`ls-border-edit.sh` 不会生成世界，跑之前必须先 `SCENARIO_KEEP_WORLD=0 bash ls-border-scenario.sh gen <tag>`
  建一次世界，否则脚本静默无输出 —— 这个坑本轮踩了两次）
...
## 8. 删除后的验收结果（2026-09-20，干净窗口，24/24 轮）

`AB_GROUP=del2 ab-interleaved.sh 3 block_toggle_border structure_cube dense_chunk_patch sky_hole`
（每档 3 轮，每轮最小值取中位观察；单位 µs）：

| 档位 | 我方 | ScalableLux | 结论 |
| --- | --- | --- | --- |
| `block_toggle_border` | 1251 / 1013 / 1018 | 2119 / 2152 / 2511 | **快 2.1×** ✓ |
| `structure_cube` | 2840 / 2485 / 1996 | 3652 / 3537 / 3744 | **快 1.4×** ✓ |
| `dense_chunk_patch` | 3008 / 2118 / 2235 | 2173 / 1770 / 2180 | 慢约 1.2× |
| `sky_hole` | 1354 / 647 / 1442 | 546 / 503 / 562 | 慢约 1.5× |

**与删除前形态完全一致**（两个领先、两个落后）→ **删除零回归** ✓，符合「移除的是默认关死代码」的预期 ✓。

**同窗口的 M1 A/B**（`syncSmallEdits` 关 vs 开）：OFF 790 / 728 µs vs ON 873 / 909 µs
→ **干净窗口里同步路径也是轻微退步（约 12–20%）** ✓ —— 与负载窗口的「无改善」方向一致，
**M1/M3 保持默认关的处置在两个窗口里都有数据支撑** ✓✓。
