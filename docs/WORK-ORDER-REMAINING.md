...
## 5. 客户端优化（用户 2026-09-19 明确要求落实；两步，第一步必须先量）

**背景**：前辈模组在客户端省的主要不是计算，而是**存储与包处理** —— 它们把客户端的光照数据结构整个换掉。
我们已经证明「客户端不算光照」（探针：`propagateLightSources` 0 次、`checkBlock` 108 次/局），
但**内存 / 簿记 / 包量这三项从未量过**（`docs/ARCH-V2-CLIENT-LIGHTING.md` §0 的更正）。

### 第一步（不换结构，今天可量可做）：把「27 格扇出」收到真正需要的那几格

现在发布时默认对每个 section 通知 3×3×3 共 27 个邻域（`notifyNeighbourSections`，默认 true）。
实测收到 1 时通知量 10015 → 1401（**7.1 倍**），**服务端中性** ✓；但客户端收到的光照数据量与重绘量
**从未量过**，而且扇出存在的理由是「邻区的网格需要重建」—— 直接关掉有跨区视觉接缝的风险。

**执行顺序（不许跳步）**：
1. **量**：起服 + 开发客户端两轮（默认 27 扇出 vs `-Dlucistarlink.notifyNeighbourSections=false`），
   用现成探针比较「客户端收到的 section 数」（`queueSectionData` 计数 × 2048 字节 ≈ 带宽）与客户端堆水位。
2. **验证视觉**：在两轮的客户端截图上检查区块/区段边界有没有光缝（全分辨率 F2 截图；桌面截图不算判据）。
3. **若两者都干净** → 把默认改成 false（一行），并发版。
4. **更外科手术式的替代方案（若第 2 步发现接缝）**：只通知「光真的越过了边界的那些邻区」——
   判据用现成的 `changesNearBorder(...)`（运行期路径已经在用它决定光环发布），
   这样保留正确性、砍掉绝大部分无用标记。**这一步动的是发布路径，必须配 26 测试 + 四项交错 + 探针闸。**

### 第二步（前辈那条路）：把客户端的光照**存储**也换掉

省的是客户端内存与区块加载/卸载簿记。代价是**与 Sodium/光影的兼容**（它们直接读原版结构；
Starlight 能共存是因为 Sodium 为它写了专门兼容路径 —— 那份成本由 Sodium 承担）。
**先量内存水位与包字节数**，收益不显著就不付这份成本。
