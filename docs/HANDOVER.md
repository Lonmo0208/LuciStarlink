...
### 判据表（三引擎一轮拉丁方）+ sky_hole 路由修复（`33d5ad9`）

| 格 | 引擎口径 我们/SL/1.x | 玩家口径（wall）我们/SL/1.x | 判据判定 |
|---|---|---|---|
| structure | 4.53 / 4.66 / **3.28** | 144 / 149 / 160 | 引擎：赢 SL（边际）、负 1.x；玩家：赢两家 |
| dense | **2.92** / 4.18 / **2.32** | 148 / **146** / 234 | 引擎：赢 SL、负 1.x；玩家：负 SL（并列） |
| sky_hole | 2.79 / 0.92 / **0.70** | 144 / 145 / **88** | 修复前全负；已路由修复（待复测） |
| border | **3.92** / 4.11 / **1.25** | 147 / 147 / 321 | 引擎：**赢 SL**、负 1.x；玩家：并列 |

**这一轮最重要的两个事实**：① **border 在引擎口径上首次赢过 SL（3.92 对 4.11）**——这一格起点是落后 2.5 倍；
② **sky_hole 是新回归**（2.79 对 0.92），根因是独占所有权把微小 burst 也交给了通道：分段显示 extNanos
1.0-1.4 ms/pass（一次基础引擎写入标记整块 24 个 section，通道整批重采纳）。**修复**：单区域、无已采纳状态、
且 <64 处的 burst 走 **grouped nibble 路径（同一 flush 内同步——不是队列，所以没有异步覆盖）**；
集中/多区域/已采纳区域不变。门绿。

**vs 1.x：本轮 0/4**（structure 4.53/3.28、dense 2.92/2.32、sky_hole 2.79/0.70、border 3.92/1.25）。

**环境教训（本轮丢了若干次运行）**：残留 java 会持有 mods 里的 jar，`rm -f`/`cp -f` 在 Windows 上**静默失败**
→ 服务端加载上一个引擎的 jar → "expected mod is not loaded" + 空结果（看起来像慢跑）。`stage_jar`（md5 校验 +
重试 + 之间 kill_strays）现已进入每个 rig 脚本。**跑判据表前请重启机器或至少清空 java**——本次会话末尾环境已退化到
8 次运行仅 3 次成功。
