...
### 判据复测（清环境）+ sky_hole 三次路由尝试失败（`0196d94`）

清环境（只有 gradle daemon、端口全空——之前的"3/8 空结果"是我自己的 kill 逻辑与在飞服务端撞车，不是机器坏了）：

| 格 | 我们 | SL | 判定 |
|---|---|---|---|
| structure | 4.53 | 4.66 | 赢（边际） |
| dense | **2.75-3.77** | 3.73-3.96 | **赢** |
| border | 4.04-6.62 | 4.12-4.67 | 互有胜负（**判据表那轮的 3.92 对 4.11 是窗口依赖的**） |
| sky_hole | 2.32-3.42 | **0.73-1.42** | **大幅落后** |

**sky_hole 的三次路由尝试，全部失败**（每条都实测）：① <64 处且单区域 → nibble：**没生效**（它的 25 处跨多个区域）；
② 再要求 <8 区域 → nibble：**仍生效**（通道还会捕获箱体附近已加载区块的世界生成写入）；③ 区域 3×3 邻域必须过 LIGHT：
**仍生效**。读数依次 2.79 → 1.56/1.97 → 2.32/3.78 → 2.32/3.42。规则保留（保守、门绿、默认关），但结论是：

**下一步不是再加规则，而是诊断**——用 `-Dscalablelux.imageLaneDebug` 跑一次 sky_hole，读清楚**通道到底接管了哪个区域、
哪一批 burst**（计数器已在树里）。在看清之前继续加启发式规则只会重犯这三条的错误。

**对外口径补充**：`border` 的"赢过 SL"目前**不能作为定论**（一个窗口 3.92 对 4.11、另一个窗口 4.04-6.62 对 4.12-4.67）；
`dense` 的赢面在两个窗口都成立；`sky_hole` 在通道开启时稳定落后，**开启通道前必须解决它**。

### 诊断的回报：一个真 bug + 路由规则定案（`0057191`）

**`imageLaneDebug` 一跑就露馅**：通道在 sky_hole 上结算的是 **59 个不同区域、每个 1-6 处改动**（172 次结算、2208 个
section 物化——箱体附近的世界生成写入），也就是说之前三条路由规则一直在放错误的流量进来。顺藤摸瓜抓到真 bug：
**`materializeForChanges` 用未加垫的 depth 和 off-by-one 的 y 去解码带垫索引**，reach 盒建在错误的格子上、
物化了错误（通常更少）的 section。暴露它的是"墙上脏标记"那个临时诊断（它在抛异常、直接杀掉整轮运行——我自己的空结果）；
现在改为报告一次并返回。

⚠ **必须撤回一条结论：判据表那轮的「border 3.92 赢过 SL 4.11」是在这个 bug 下测的**（少物化 = 少干活）。

**路由规则定案（按区域）**：通道只接管"该区域自己的 burst ≥ 64 处改动"。中途试过的"flush 总量"规则更差——
它会放行 border（95 处散在 19 个区域、每区约 5 处），而通道的每区域机制（物化解码修正后）要 6.7 ms/pass 对
grouped nibble 路径的约 4.5。`covers()` 必须与结算算同一个量（传 0 会让 flush 在 nibble 路径播种方块半边、
通道又结算一次——双份干活，border 4.3 → 10.7）。
