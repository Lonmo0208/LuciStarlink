...
**1.25×**（我方中位 2595 µs / SL 中位 2069 µs）。第二组（`master` 构建，3 轮）见 `docs/TASK-PERF-SKY.md` §10.12。
严格证明修复本身的 jar-A/B（旧 jar 与新 jar 交错）**尚未跑**，所以"1.83× → 1.25×"是两次不同构建/时段之间的
对比，属于弱证据；同场 5 轮的结论（赢 border、输 dense/sky）才是强证据。
...
发布线 `master` 上是 **26 通过 / 0 失败 / 1 继承跳过**（原 21 + 新 5）；V2 分支另有 3 个存储接缝测试，
所以那边是 29 —— 两边的数字都对，别混。

**分支事记**：1.1 的改动先在 `v2-storage` 上提交（`3c43446`）再 cherry-pick 到 `master`，cherry-pick 干净。
发布件由 `master` 构建（**不含** V2 的读接缝）；`docs/verify-baseline.txt` 里的 5 轮性能表则是在带接缝的构建上
测的（默认 region 模式，功能等价于 1.0.3 行为 + 1.1 修复），随后在 `master` 上跑了一组 3 轮确认。
