...
> **执行计划见 `docs/V2-PLAN.md`**（阶段划分与本文件一致：0–5；那里有任务清单、要改的文件清单、熔断条件、
> 工作量估算与更新过的验收口径）。本文件是**设计**；下面两处已过期，以 `V2-PLAN.md` 为准：
> ① 本仓库**已有 git**（1.0.0 发布后共 40 个提交），"开工前必读"的第 1、2 件事均已完成；
> ② §6 验收矩阵里的 `sky_hole min ≤ 3.0 ms` 是**已废弃的绝对线** —— 现行口径是"同场逐轮交错、≥5 轮、
> `minPassNanos` 中位数、附精确 p、比值需独立复现一次"，且每组必须带 `prepareRing=3` + `quiesceSettleMs=1000`；
> `fourway.sh` 亦已被 `fourway-interleaved.sh` 取代（顺序组不可比）。
