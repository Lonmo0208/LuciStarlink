...
**1.2.10** —— 服务端引擎、跨区正确、内存有界、存档安全、重启截断缺陷已修（1.1.5）、
客户端收到的光照流量减半（1.2.5）、世界生成的自然光源光照与原版逐位一致（1.2.7）、
**大范围 fill 回空气后的残留光照已修**（1.2.9，社区 PR #2，本仓库用可复现探针独立验证）、
**加权性能：对 ScalableLux = 0.9339（快 6.6%），对上游 Lucis = 0.929（快 7.1%）** —— 六轮三方交错，
见 `docs/TASK-PERF-SKY.md` §18；脏环境下的退化与原因见 §16。

**干净窗口 vs 被压垮的机器**：健康条件下我们在小改动档与 ScalableLux 同级（589 vs 573 µs）、在两个重负载档领先
**（按引擎计算量；那个口径刻意排除了光照要跨的 tick 边界）**；而 `structure_cube` 上玩家能感觉到的落地延迟是
**SL 更快 —— 66.5 ms/趟对 99.1 ms/趟**（同量具、同会话交替，见 `docs/TASK-PERF-SKY.md` §19）；
机器被压垮时小改动档落后，因为整个服务器 tick 被调度得更慢 —— 五变体实测结论见 `docs/ARCH-V3-SYNC-STORAGE.md` §6。
**但按同一条三方脚本的默认协议，权重最大的 `block_toggle_border` 是真的落后**：单独复现 12+12 轮读
**1.2954（U=113/31，p=0.0173）**，上游 Lucis 同样落后（1.3007）；加权仍是我们赢，因为赢的那一档
（`structure_cube` 0.819）绝对值最大、权重 35% —— 引用 0.9339 时请连逐档 1 胜 3 负一起给，证据在
