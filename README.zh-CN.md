...
以下数字来自本项目自己的测试台（`run-benchmark-scalablelux`）：同一个会话、三方**交替**运行（三个引擎按轮次轮换）、
同一张地图、固定种子、每次测量前世界已静默；每个引擎取 3 轮的中位数。**这是 2.0.4 修复之后的表**——下面那条说明必须一起读，
因为 2.0.4 之前的那份表有一个"让本引擎显得更快"的错误。

**引擎口径** —— `minPassNanos`，引擎自己在一个 pass 里的工作量（取最好的那个 pass，避开跨 tick 噪声）：

| 档位（玩家的说法） | **LuciStarlink 2.0.4** | ScalableLux | 1.x（Lucis 线） | 结论 |
|---|---|---|---|---|
| `block_toggle_border` —— 沿区块边界快速拆放 | 5.00 ms | 4.18 ms | **1.22 ms** | **输**，两家都比我们快 |
| `structure_cube` —— 盖一个实心建筑 | **2.78 ms** | 4.62 ms | 2.97 ms | **赢**两家 |
| `dense_chunk_patch` —— 大面积改动 | **1.07 ms** | 4.07 ms | 2.35 ms | **赢**两家 |
| `sky_hole` —— 一次很小的改动 | **0.69 ms** | 0.83 ms | 1.06 ms | **赢**两家 |

作为参照，**原版**（不装任何光照模组）在同一个测试台上三档重负载约 10–11 ms、`sky_hole` 约 1.0 ms —— 那是另一个会话量的，
按数量级引用。

**玩家口径** —— `bench.pass_wall_actual`，一个 pass 的墙钟时间，**包含**玩家要等的跨 tick（中位数 / 最好一轮）：

| 档位 | LuciStarlink 2.0.4 | ScalableLux | 1.x |
|---|---|---|---|
...
这份数据的窗口是**带负载的**（CPU 平均 30.3%、峰值 48.7% —— 本项目的测量机按设计就跑着别的东西）。让对比成立的是
**同会话交替**；绝对值是"玩家体感"的图景，不是安静机房里的数字。

**诚实读法：**

- **两胜、一负、一条地板线。** `structure_cube`（2.78 对 4.62 / 2.97）、`dense_chunk_patch`（1.07 对 4.07 / 2.35）、
  `sky_hole`（0.69 对 0.83 / 1.06）三档**对两家都赢**；`block_toggle_border` **对两家都输**（5.00 对 ScalableLux 4.18、
  1.x 1.22）——那一格是小批量同步作业，而 1.x 的引擎本来就是同步的。
- **玩家口径四档都在一个 tick 的地板上**（48–50 ms），ScalableLux 也是；1.x 只有 `structure_cube` 的最好一轮到这个水平，
  其余三档慢 30–50 ms。
- **不是速度，而是正确性**：本引擎产出的光照在测量箱上与**原版和 ScalableLux 逐位相同**（每次运行都打印指纹
  `sky=905931078dfc5ace`）；1.x 线的是 `641356fc41163add`，与两者都不相同。
- **引用更早的表之前必须知道这件事**：2.0.4 之前本引擎 `block_toggle_border` 的读数是 0.32 ms，招牌说法是"比 ScalableLux
  快 12 倍"。那次测量是在那一格的方块光传播**根本没做**的情况下取的——就是让"放置的火把只亮自己一格"的同一个缺陷
  （`docs/BUG-EMITTER-BLOCK-LIGHT.md` 根因四）。0.32 是假象，本 README 不再引用它。
- **`structure_cube` 这一格必须随数字一起说明的口径**：那一格约 **90% 的读数**是 Minecraft 自己的 `Level.setBlockState`
  （写状态、高度图、标脏、原版同步的光照钩子）——**每个**引擎量到的都是 600–775 ns/改动，原版也一样。这一格不可能出现大比分胜利。
