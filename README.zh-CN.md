...
3. **合并的减光抽干**：每个受影响的区块只做播种，整批改动结束后**只走一遍**引擎的减光队列（`batchDecrease`）。
   逐区块抽干走的是全局队列，每次 220–430 µs。
...
把 1–4 打开的三个开关**默认都是开的**；要关掉可以手动指定（`-Dscalablelux.ownEdit=false` 等，见"配置"）。
...
| `-Dscalablelux.batchDecrease` | **`true`** | 合并的减光抽干（机制 3） |
